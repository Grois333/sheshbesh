export const calculateLegalMoves = (chips, dice, turn, whiteJail, blackJail)=>{
  ...
  //if we are in jail
  //calculate space in 0-5 or 23-18

  if( (turn === 'white') && (whiteJail > 0) ){
    //check if 23-18 are legal legalMoves
    ...
    // if all dice we have, for all the chips we have, check if its open
    ...
    const legalHomeMoves = (furthestPiece > 6 ) ? [] : ( turn === 'white' ) ? [0, 1, 2, 3, 4, 5].filter (spot => (
       ( chips[spot] < 0) && (
  ...
  // if if all we have, for all the chips we have, check if its open
};
